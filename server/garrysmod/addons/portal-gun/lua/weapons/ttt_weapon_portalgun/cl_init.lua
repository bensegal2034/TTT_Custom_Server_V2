...
local VElements = {
	...
	["InsideEffects"] = { type = "Sprite", sprite = "sprites/portalgun_effects", bone = "ValveBiped.Front_Cover", rel = "", pos = Vector(0, -2.201, 0), size = { x = 0.025, y = 0.025 }, color = Color(255, 255, 255, 255), nocull = false, additive = true, vertexalpha = true, vertexcolor = true, ignorez = false}
}
...
local WElements06 = {
	["BodyLight"] = { type = "Sprite", sprite = "sprites/betalight", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(7, 1.40, -4.801), size = { x = 0.03, y = 0.03 }, color = Color(255, 255, 255, 255), nocull = false, additive = true, vertexalpha = true, vertexcolor = true, ignorez = false}
}
...
function SWEP:Initialize()
	...
	self.WElements = table.FullCopy( WElements06 )
	else
	self.VElements = table.FullCopy( VElements )
...
local VBodyLights = {
	{bone = "ValveBiped.Base", pos = Vector(0.25, -5.45, 10.5), size = { x = 0.018, y = 0.018 }}
}

local VBlueLight = Material("sprites/bluelight")
local VBlueLightB = Material("sprites/bluelightb")
local VOrangeLight = Material("sprites/orangelight")
local VOrangeLightB = Material("sprites/orangelightB")
local VNoLight = Material("sprites/blanklight")
...
function SWEP:ViewModelDrawn(vm)
	...
	for k,v in pairs(VBodyLights) do
		local bone = vm:LookupBone(v.bone)
		...
		render.SetMaterial(VBlueLightB)
		elseif last == TYPE_BLUE then
		render.SetMaterial(VBlueLight)
		elseif last == TYPE_ORANGE and GetConVarNumber("portal_beta_borders") >= 1 then
		render.SetMaterial(VOrangeLightB)
		elseif last == TYPE_ORANGE then
		render.SetMaterial(VOrangeLight)
		elseif last == 0 then
		render.SetMaterial(VNoLight)
		end
		render.DrawSprite(drawpos, v.size.x*128+_sin, v.size.y*128+_sin, col)
		end
	...
	for k, name in pairs( self.VElements ) do
		...
		if (v.type == "Sprite" and sprite) then
			...
			local col = Color(255,255,255)
			if last == TYPE_BLUE and GetConVarNumber("portal_beta_borders") >= 1 then
			col = Color(64,235,255,255)
			elseif last == TYPE_BLUE then
			col = Color(64,160,255,255)
			elseif last == TYPE_ORANGE and GetConVarNumber("portal_beta_borders") >= 1 then
			col = Color(218,64,120,255)
			elseif last == TYPE_ORANGE then
...
--	BobTimeLast = CT
	local BobCycleX = sin(BobTime * 0.5 % 1 * pi * 2) * 1 --BobCycleMultiplier
	local BobCycleY = sin(BobTime % 1 * pi * 2) * 1 --BobCycleMultiplier
...
local cOrgB = Color(218,64,120,255)
