...
function SWEP:SecondaryAttack()
	if self:Clip1() > 0 then
		if IsValid(self:GetNW2Entity("MeleeTargetH3")) then LungeRange = (self:GetNW2Entity("MeleeTargetH3"):GetPos() - self.Owner:GetPos()):Length() end
		...
		if not IsValid(self:GetNW2Entity("MeleeTargetH3")) or LungeRange > self.LungeDistance then self:MeleeHitSword() end
		self:SetNextIdle(0)
	...
	elseif self:Clip1() <= 0 then
		...
		if not IsValid(self:GetNW2Entity("MeleeTargetH3")) or LungeRange > self.LungeDistance then self:MeleeHit() end
		self:SetNextIdle(0)
		timer.Stop("GrenadeThrow" .. self:EntIndex())
		timer.Stop("weapon_idle" .. self:EntIndex())
		self:SendWeaponAnim(ACT_VM_HITCENTER)
...
function SWEP:Deploy()
	...
	if self:GetNW2Bool("FreshSword") == true then self:SetNW2Bool("FreshSword", false) end
	self:SetSkin(1)
	...
	if self:Clip1() <= 0 then self.Owner:GetViewModel():SetSkin(1) end
	if self:Clip1() > 0 and game.SinglePlayer() == false and IsFirstTimePredicted() then
		...
		timer.Create("SwordActivation" .. self:EntIndex(), 0.56, 1, function()
			if IsFirstTimePredicted() then
				self.Owner:GetViewModel():SetSkin(0)
				self:SetSkin(0)
				self:EmitSound("Halo3_ES.Draw")
				util.Effect("flash_sworddraw_halo3", effectdata)
				util.Effect("flash_sworddraw_halo3", effectdata)
			end
		end)
	elseif self:Clip1() > 0 and game.SinglePlayer() == true and IsFirstTimePredicted() then
		...
		timer.Create("SwordActivation" .. self:EntIndex(), 0.66, 1, function()
			if IsFirstTimePredicted() then
				self.Owner:GetViewModel():SetSkin(0)
				self:SetSkin(0)
				self:EmitSound("Halo3_ES.Draw")
...
function SWEP:PrimaryAttack()
	...
	if self.Owner:KeyDown(IN_USE) and self:Ammo2() > 0 and self.GrenadeEnabled then
		...
		if not IsValid(self:GetNW2Entity("MeleeTargetH3")) and self:Clip1() > 0 or IsValid(self:GetNW2Entity("MeleeTargetH3")) and LungeRange > self.LungeDistance and self:Clip1() > 0 then
			self:SetNextIdle(0)
			timer.Stop("GrenadeThrow" .. self:EntIndex())
			timer.Stop("weapon_idle" .. self:EntIndex())
			self:MeleeHitSword()
		...
		if not IsValid(self:GetNW2Entity("MeleeTargetH3")) and self:Clip1() > 0 and not self.Owner:OnGround() then
			self:SetNextIdle(0)
			timer.Stop("GrenadeThrow" .. self:EntIndex())
			timer.Stop("weapon_idle" .. self:EntIndex())
			self:MeleeHitSword()
...
function SWEP:Holster()
	if IsValid(self.Owner) and self.Owner:IsNPC() then return end
	if self.FirstDraw == false then self:StopSound(self.SwordIdleSound) end
	...
	if IsValid(self.Owner) and self.Owner:IsBot() and self.Owner:GetActiveWeapon() == self then self.Owner.SetBotSpeedH3 = true end
	...
	if IsValid(self.Owner) and self.Owner:IsNPC() and SERVER then
		...
		timer.Simple(1, function()
			if IsValid(self) and IsValid(self.Owner) then
				self.Owner:Give("h3_energysword_swep_ai")
				if IsValid(self.Owner:GetWeapon("h3_energysword_swep_ai")) then
					if self:Clip1() < self.Primary.ClipSize or self:GetNW2Int("NPCClipH3") ~= -1 then self.Owner:GetWeapon("h3_energysword_swep_ai").Used = true end
					if self:GetNW2Int("NPCClipH3") ~= -1 then
						self.Owner:GetWeapon("h3_energysword_swep_ai"):SetClip1(self:GetNW2Int("NPCClipH3"))
					else
						self.Owner:GetWeapon("h3_energysword_swep_ai"):SetClip1(self:Clip1())
					end
				end
...
function SWEP:Think()
	if self.Owner:InVehicle() and self.Owner:GetAllowWeaponsInVehicle() == false then
		self:StopSound(self.SwordIdleSound)
		return
	end
	...
	if IsValid(self) and IsValid(self.Owner) and self:GetSkin() == 0 and self:Clip1() > 0 and IsFirstTimePredicted() then self:EmitSound(self.SwordIdleSound) end
