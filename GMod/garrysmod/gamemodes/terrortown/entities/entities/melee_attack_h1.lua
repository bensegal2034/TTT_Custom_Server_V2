...
AddCSLuaFile( "melee_attack_h1.lua" )
...
local target = data.HitEntity
	...
	target:TakeDamage(30,self.Owner,self.Owner)
	target:EmitSound("halo1/melee_hit_" .. math.random(1,2) .. ".ogg")
	...
	self.FakePredicted = true
	if self.FakePredicted == false then SafeRemoveEntity(self) end
	target.Owner:TakeDamage(30,self.Owner,self.Owner)
	target.Owner:EmitSound("halo1/melee_hit_" .. math.random(1,2) .. ".ogg")
	SafeRemoveEntity(self)
	elseif self:EntityIsVehicle(target) and self.FakePredicted == false and IsValid(target) and IsValid(self.Owner) then
	self.FakePredicted = true
	if self.FakePredicted == false then SafeRemoveEntity(self) end
	target:TakeDamage(40,self.Owner,self.Owner)
	self:EmitSound("physics/concrete/rock_impact_hard4.wav", 80)
	SafeRemoveEntity(self)
...
if ent:IsNextBot() and self.FakePredicted == false and IsValid(ent) and IsValid(self.Owner) then
	self.FakePredicted = true
	if self.FakePredicted == false then SafeRemoveEntity(self) end
	ent:TakeDamage(30,self.Owner,self.Owner)
	ent:EmitSound("halo1/melee_hit_" .. math.random(1,2) .. ".ogg")
	SafeRemoveEntity(self)
end
