...
hook.Add("DoPlayerDeath", "ZapperDieSfx", function(ply, atkply, dmg)
   if not(ply.GetActiveWeapon) then return end
   local wep = ply:GetActiveWeapon()
   if IsValid(wep) and wep:GetClass() == "weapon_ttt_zapper" then
      ply:EmitSound("weapons/zapper/die".. math.random(11, 12) .. ".wav")
   end
end)
