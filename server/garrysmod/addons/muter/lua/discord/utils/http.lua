  ...
  http.Fetch(httpsAdress .. "/" .. req,
    function(res)
      if (util.JSONToTable(res).errorMsg) then
        print_debug("[" .. GetConVar("discord_name"):GetString() .. "][Error] " .. util.JSONToTable(res).errorMsg)
      end
      callback(util.JSONToTable(res))
    end,
    function(err)
      print_debug("[" .. GetConVar("discord_name"):GetString() .. "] Request to bot failed to respond. Is the bot running? Or is the URL correct?")
      print_debug("[" .. GetConVar("discord_name"):GetString() .. "][Error] " .. err)
