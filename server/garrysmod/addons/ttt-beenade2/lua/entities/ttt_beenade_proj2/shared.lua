...
ENT.Model = Model("models/lucian/props/stupid_hive.mdl")

--Change these values to modify damage proporties
ENT.ExplosionDamage = 0
ENT.ExplosionRadius = 0

beeCounter = 12 --Number of bees at a time
beePollRate = 5 --Number of seconds between bee spawn checks
maxBees = 60 --Max number of bees that can be spawned in total

totalBees = 0 --Total number of bees spawned

BeesList = {} --Currently active bee entities
...
--Initialize the grenade/hive entity
function ENT:Initialize()
	
	--Set variables based on console variables
	if SERVER then
		BeeInnocentDamage 	= GetConVar("bee_innodamage"):GetInt()
		BeeTraitorDamage	= GetConVar("bee_traitordamage"):GetInt()
		beePollRate			= GetConVar("bee_spawnrate"):GetInt()
		maxBees				= GetConVar("bee_totalcount"):GetInt()
		killableHive		= GetConVar("bee_hivekillable"):GetInt()
		hiveHP				= GetConVar("bee_hivehealth"):GetInt()
	end
	...
	if SERVER then
		self:SetMaxHealth(hiveHP)
		self:SetHealth(hiveHP)
	end
	
end


--Trigger explosion and start spawning bees
function ENT:Explode(tr)
	
	if SERVER then --Server-side code
		totalBees = 0
		
		--Set number of bees to be spawned
		if GetConVar("bee_random"):GetInt() == 0 then
			beeCounter = GetConVar("bee_concurrent"):GetInt()
		else
			beeCounter = math.random(GetConVar("bee_randommin"):GetInt(),GetConVar("bee_randommax"):GetInt())
		end
		
		--Limit bees to be spawned to the max amount
		if beeCounter > maxBees then
			beeCounter = maxBees
		end
		...
		--Check if all bees have spawned, and if not, continue spawning
		if totalBees < maxBees then
			timer.Simple( 5, function() SpawnMoreBees(pos, BeeNPCClass, self) end)
		else
			self:Remove()
		end
...
--Allow hive to take damage, and change color
function ENT:OnTakeDamage(dmginfo)

	if killableHive then
		self:SetHealth(self:Health() - dmginfo:GetDamage())
		
		self:SetColor(Color(255, (255*(self:Health()/self:GetMaxHealth())), (255*(self:Health()/self:GetMaxHealth())), 255))
...
--Spawn more bees (runs recursively)
function SpawnMoreBees(hivepos, BeeNPCClass, thisThing)
	
	--Check if round has restarted
	if not thisThing:IsValid() then
		return
	end
	
	--Remove dead bees from list of bees
	local k = 1;
	while k <= #BeesList do
		if not BeesList[k]:IsValid() then		
			table.remove(BeesList,k)
		else
			k = k + 1
		end
	end

	--Spawn new bees to replace dead ones TODO cleanup
	for i=#BeesList+1,beeCounter do
		
		if totalBees >= maxBees then
			break
		end
	
		local spos = hivepos+Vector(math.random(-75,75),math.random(-75,75),math.random(0,50))
		local contents = util.PointContents( spos )
		
		local headBee = SpawnNPC(thisThing:GetThrower(),spos, BeeNPCClass)
		...
		headBee:SetNWEntity("Thrower", thisThing:GetThrower())
		--headBee:SetName(self:GetThrower():GetName())
		headBee:SetNoDraw(true)
		headBee:SetHealth(1000)
	...
	--Check if all bees have spawned, and if not, continue spawning
	if totalBees < maxBees then
		timer.Simple( 5, function() SpawnMoreBees(hivepos, BeeNPCClass, thisThing) end)
	else
		thisThing:Remove()
	end
...
if SERVER and GetConVarString("gamemode") == "terrortown" then
	...
	function BeeNadeDamage(victim, dmg)
		...
		if attacker:IsValid() and attacker:IsNPC() and attacker:GetClass() == BeeNPCClass and victim:IsPlayer() then
