...
function ENT:ZipZap(v)
	if SERVER then
		...
		timer.Simple(3, function () -- Change timer length here if you want the victim to be ragdolled for a different amount of time.
			local pos1 = vragdoll:GetPos()
  		   ...
  		   for a,b in pairs( respawnweapons ) do
    	       print("[SHOCK] Check wep ", b )
	           if !v:HasWeapon( b ) then
	               v:Give( b )
	               if( b == curwep ) then
            ...
            timer.Simple(8, function() v:SetWalkSpeed(oldspeed) end) -- Change timer length here if you want the victim to be slowed for a different amount of time.
            end)
		end
