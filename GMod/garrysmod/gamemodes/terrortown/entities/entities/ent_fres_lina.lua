...
local NoHitEnts = {
	["func_breakable_surf"] = true, -- This is stuff we shouldn't attach to, glitches out
	["ent_fres_lina"] = true,
}
...
function ENT:PhysicsCollide(data, phys)
	if self.toc == false and self.f == false then
		...
		self.toc = true
		local phys = self:GetPhysicsObject()
		if phys:IsValid() then phys:SetVelocity(Vector(0, 0, 0)) end
...
function ENT:Think()
	if SERVER then
		...
		if c > GetConVar("frest_range"):GetInt() and self.toc == false then self.f = true end
		if self.f == true then
			if c < 250 then self:Remove() end
			if self:GetMoveType() ~= MOVETYPE_VPHYSICS then self:SetMoveType(MOVETYPE_VPHYSICS) end
			local d = a - b
			local phys = self:GetPhysicsObject()
			if IsValid(phys) then
				phys:EnableGravity(false)
		...
		if self.toc == true then
			...
			if c > GetConVar("frest_range"):GetInt() or self.tim <= CurTime() then
				self.f = true
				self.toc = false
			end
